...
- **Provider 兼容**：[provider-compat.md](provider-compat.md)（ironclaw、zeroclaw、openclaw）
...
- **配置与数据**：`~/.cursor-brain/` 为配置与数据根目录；支持 bind_address、minimal_workspace_dir、agent_mode、sandbox、allow_agent_write。
...
- **配置项**：port、bind_address（默认 0.0.0.0）、request_timeout_sec、session 相关、default_model、fallback_model、cursor_path、minimal_workspace_dir（默认 ~/.cursor-brain/workspace）、agent_mode（ask|agent）、sandbox（enabled|disabled）、allow_agent_write（是否传 --force）、forward_thinking（off|content|reasoning_content，默认 content）。
...
- **agent_mode**：`ask` 只读、`agent` 完整能力；纯聊天可配 `ask` 降低写操作。
- **sandbox**：enabled（默认）/ disabled。
- **allow_agent_write**：false 时不传 --force，agent 不直接落盘，由上层决定是否执行。
...
| `agent_mode`                       | "agent"                   | "ask" \| "agent".                                                        |
| `sandbox`                          | "enabled"                 | "enabled" \| "disabled".                                                 |
| `allow_agent_write`                | true                      | If false, do not pass --force.                                           |
